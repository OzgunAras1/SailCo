...
@variable(m, dec[1:4] <= 0) # althought other samples, i have dec negative, adding with inc

@variable(m, incH[1:5] >= 0)
@variable(m, decH[1:5] <= 0)

@constraint(m, h[1] == 10) # we have 10 before the process

@constraint(m, x[1] == 40) # 50 total will be produced before the process
@constraint(m, y[1] == 10)

@constraint(m, incH[5] >= 10)
@constraint(m, decH[5] == 0)

@constraint(m, invH[i in 1:5], h[i] == incH[i] + decH[i]) # inventory in Q = incH + decH

@constraint(m, inv[i in 1:4], h[i]+x[i+1]+y[i+1] == d[i] + h[i+1]) 

@constraint(m, incdec[i in 1:4], (x[i+1]+y[i+1]) - (x[i]+y[i]) == (inc[i] + dec[i])) # Q2- Q1 production difference = inc + dec
